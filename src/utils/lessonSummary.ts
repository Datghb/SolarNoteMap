...
export async function askLessonSummaryAI(lessonId: string, questionValue: string, history: SummaryChatMessage[], signal?: AbortSignal, pdfUrl?: string) {
  ...
  const response = await fetch('/api/lesson-summary-chat', {
    ...
    body: JSON.stringify({ lessonId, question, history: safeHistory, ...(pdfUrl ? { pdfUrl } : {}) }),
    signal,
  });
  const payload = await response.json().catch(() => ({}));
