...
const DB_TO_ACTIVITY: Record<string, StudentActivity['type']> = {
  lesson_viewed: 'lesson_opened', note_created: 'note_updated', map_created: 'map_saved', question_posted: 'question_posted', answer_posted: 'answer_posted',
};
const ACTIVITY_TO_DB: Record<StudentActivity['type'], string> = {
  lesson_opened: 'lesson_viewed', slide_viewed: 'lesson_viewed', note_updated: 'note_created', map_saved: 'map_created',
  question_posted: 'question_posted', answer_posted: 'answer_posted', understanding_updated: 'lesson_viewed',
};
...
export async function createClassroom(name: string, description: string, joinCode: string) {
  const { data, error } = await requireSupabase().rpc('create_class_with_code', { class_name: name.trim(), class_description: description.trim(), join_token: joinCode.trim() });
  if (error) throw error;
  if (typeof data !== 'string') throw new Error('Không thể tạo lớp học.');
  return data;
}
...
export async function loadCloudActivities(classId: string): Promise<StudentActivity[]> {
  ...
  return (data ?? []).map((row) => {
    ...
    return { id: String(row.id), studentId: row.user_id, studentName: profile?.display_name ?? 'Học sinh', lessonId: row.lesson_id ?? '', type: DB_TO_ACTIVITY[row.kind] ?? 'lesson_opened', occurredAt: row.occurred_at, metadata: row.metadata };
  });
}
...
export async function recordCloudActivity(classId: string, lessonId: string, type: StudentActivity['type'], metadata: Record<string, unknown> = {}) {
  ...
  if (!auth.user) return;
  const { error } = await client.from('student_activities').insert({ user_id: auth.user.id, class_id: classId, lesson_id: lessonId || null, kind: ACTIVITY_TO_DB[type], metadata });
  if (error) throw error;
}
...
export async function saveCloudMap(lessonId: string, title: string, graph: unknown) {
  ...
  const { data: auth } = await client.auth.getUser();
  if (!auth.user) return;
  const { data: existing } = await client.from('knowledge_maps').select('id').eq('lesson_id', lessonId).eq('user_id', auth.user.id).maybeSingle();
  const payload = { lesson_id: lessonId, user_id: auth.user.id, title, graph };
  const query = existing?.id ? client.from('knowledge_maps').update(payload).eq('id', existing.id) : client.from('knowledge_maps').insert(payload);
